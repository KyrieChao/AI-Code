package com.chao.aicode.langgraph4j.tools;
...
import com.chao.aicode.langgraph4j.model.enums.ImageCategoryEnum;
...
public class MermaidDiagramTool {
    ...
    public List<ImageResource> generateMermaidDiagram(@P("Mermaid 图表代码") String mermaidCode,
        ...
        try {
            ...
            // 上传到COS
            String keyName = String.format("AI-Code/mermaid/%s", diagramFile.getName());
            String cosUrl = cosManager.uploadImageIfValid(keyName, FileUtil.getInputStream(diagramFile), diagramFile.length());
            // 清理临时文件
    ...
    private File convertMermaidToSvg(String mermaidCode) {
        ...
        if (!tempOutputFile.exists() || tempOutputFile.length() == 0) {
            throw new BusinessException(HTTPResponseCode.SYSTEM_ERROR, "Mermaid CLI 执行失败");
        }
        // 清理输入文件，保留输出文件供上传使用
