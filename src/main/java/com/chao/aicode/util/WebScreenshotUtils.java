...
public class WebScreenshotUtils {
    ...
    // 全局静态初始化，避免重复初始化驱动程序：
    static {
        final Integer DEFAULT_WIDTH = 1600;
        final Integer DEFAULT_HEIGHT = 900;
    ...
    private static WebDriver initChromeDriver(int width, int height) {
        try {
            ...
            // 配置 Chrome 选项
            ChromeOptions options = getOptions(width, height);
            // 创建驱动
            ...
            throw new BusinessException(HTTPResponseCode.SYSTEM_ERROR, "初始化 Chrome 浏览器失败");
        }
    }

    private static ChromeOptions getOptions(int width, int height) {
    ...
    private static void saveImage(byte[] imageBytes, String imagePath) {
        try {
            ...
            throw new BusinessException(HTTPResponseCode.SYSTEM_ERROR, "保存图片失败");
        }
    }
    ...
    private static void compressImage(String originImagePath, String compressedImagePath) {
        ...
        try {
            ...
            throw new BusinessException(HTTPResponseCode.SYSTEM_ERROR, "压缩图片失败");
        }
    }
