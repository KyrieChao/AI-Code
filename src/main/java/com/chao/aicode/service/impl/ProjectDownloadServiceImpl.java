...
public class ProjectDownloadServiceImpl implements ProjectDownloadService {
    ...
    public void downloadProjectAsZip(String projectPath, String downloadFileName, HttpServletResponse response) {
        // 基础校验
        ThrowUtils.throwIf(StrUtil.isBlank(projectPath), HTTPResponseCode.PARAM_ERROR, "项目路径不能为空");
        ThrowUtils.throwIf(StrUtil.isBlank(downloadFileName), HTTPResponseCode.PARAM_ERROR, "下载文件名不能为空");
        File projectDir = new File(projectPath);
        ThrowUtils.throwIf(!projectDir.exists(), HTTPResponseCode.PARAM_ERROR, "项目路径不存在");
        ThrowUtils.throwIf(!projectDir.isDirectory(), HTTPResponseCode.PARAM_ERROR, "项目路径不是一个目录");
        ...
        try {
            ...
            throw new BusinessException(HTTPResponseCode.SYSTEM_ERROR, "打包下载项目失败");
        }
    }
